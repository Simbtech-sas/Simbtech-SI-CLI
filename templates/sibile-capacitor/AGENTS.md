...
A **Capacitor + Vite** mobile app that talks to a SiSAAS backend.
...
Tokens live in **Capacitor Preferences backed by the Keychain/Keystore**, never localStorage — a webview's localStorage is readable.
...
5. **Finish means tested.** pnpm typecheck && pnpm build, then run it in the native shell — actually tap through the flow. Leave a test
