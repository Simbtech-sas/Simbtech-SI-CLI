...
A **Flutter** mobile app that talks to a SiSAAS backend.
...
Tokens live in **flutter_secure_storage** — Keychain and Keystore, never SharedPreferences.
...
5. **Finish means tested.** flutter analyze && flutter test, then run it on a device — actually tap through the flow. Leave a test
