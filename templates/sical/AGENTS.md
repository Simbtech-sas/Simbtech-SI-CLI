...
```bash
si list tools            # local-first entries only, for this flavor
si upgrade --dry-run     # what a newer template would change
```

Check the licence and the offline guard before adding anything: a dependency
that phones home breaks the one promise this flavor makes.
