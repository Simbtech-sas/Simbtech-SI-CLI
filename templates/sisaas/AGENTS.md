...
## 2. Use the CLI. It knows things you do not.

This project was generated by `si`, and `si` is how you change its shape. Full
reference: **`docs/SI-CLI.md`**. `si <command> --help` is authoritative.

```bash
si scaffold <Entity> -m <module>   # a feature module, wired end to end
si add <tool>                      # deps + compose + env + registration
si start dev                       # containers, migrations, API, worker, web
si upgrade --dry-run               # what a newer template would change
```

**Do not hand-write a feature module.** `si scaffold` emits the schema, a
migration numbered after whatever already exists, the repository, the service,
the DTOs, the controller — and registers the module in `app.module.ts` **and**
`worker.module.ts`. That last one is the reason this is not optional: a module
registered only in `app.module.ts` has event handlers that never run, and
nothing fails. No error, no warning. The events just stop arriving, and you find
out days later from a customer.

Write it by hand and you will get four of those six right.

**Do not hand-wire a tool.** `si add meilisearch` installs the dependency,
merges the compose service, appends the env keys, renders the module and inserts
it at the anchor. The failure mode of doing it manually is a compose service
with no env key, or a migration numbered over one that exists.

**Do not start the app with `pnpm dev`.** It runs the API and the web app but
not the worker — `worker:dev` is not in turbo's task graph. The worker is where
event delivery and background jobs live, so the outbox fills and no handler
fires. `si start dev` runs all three, and picks ports that are actually free.

**When the CLI is genuinely the wrong tool, say so and proceed by hand.** It
generates a conventional module; a feature that is not conventional should not
be forced through it. What is not acceptable is hand-writing a module that
`si scaffold` would have produced correctly, and quietly missing the worker
registration.

## 3. The simplest thing that works
...
## 7. Events, not reaching into each other
