...
This repo was generated by `si`, and `si` still knows how to change its shape.
Full reference: **`docs/SI-CLI.md`**. `si <command> --help` is authoritative.

```bash
si list tools            # what deploys with one command
si upgrade --dry-run     # what a newer template would change
```

The control plane's own operations belong in `apps/control-plane/src/` — the
web UI and the CLI both call them, so an operation added to one front end and
not the other is the drift this design exists to prevent. Add it to
`cluster/` or `services/`, and both get it.
