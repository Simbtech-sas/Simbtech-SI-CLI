...
This project was generated by `si`. Full reference: **`docs/SI-CLI.md`**;
`si <command> --help` is authoritative.

```bash
si list tools            # before writing an integration
si add <id>              # deps, config and registration together
si upgrade --dry-run     # what a newer template would change
```

Most features belong in the backend, not in three mobile apps. Check there
first — `si scaffold` in the server project is usually the right answer.
