...
export async function newProject(name: string | undefined, options: NewOptions): Promise<void> {
  ...
  // ── unfinished flavors refuse, rather than scaffolding something broken ───
  //
  // SiAPP's composition machinery is in place and verified — the flavor points
  // at the SiSAAS template, the `tenancy: single` choice prunes the migrations,
  // the tenancy module, the subdomain middleware and verify-rls.sh. What is NOT
  // done is the TypeScript layer: 26 files still carry `tenantId`,
  // `runInTenantContext` and `TenantTx`, and that is a refactor rather than a
  // flag. Scaffolding a project that cannot compile is worse than saying so.
  if (flavor.id === 'siapp' && !options.force) {
    throw new Error(
      'SiAPP is not finished. The composition works — migrations, tenancy module\n' +
        'and subdomain middleware are already pruned — but 26 TypeScript files still\n' +
        'reference tenantId and runInTenantContext, so the project will not compile.\n\n' +
        'Use `si new -f sisaas` today. Pass --force to scaffold it anyway and finish\n' +
        'the strip by hand.',
    );
  }
