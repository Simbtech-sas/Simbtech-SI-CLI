import { toCamelCase, toSnakeCase, toPascalCase } from './naming.ts';
...
export interface FieldDefinition {
  ...
  dbType: string;
  prismaType: string;
  /** Drizzle pg-core column builder, e.g. `text` or `numeric` */
  drizzleType: string;
  isRequired: boolean;
  isOptional: boolean;
  isUnique: boolean;
...
/**
 * Drizzle pg-core column builder for a field type.
 *
 * `money` maps to `numeric`, never `real`/`doublePrecision`: money must not pass
 * through IEEE-754. It surfaces in TypeScript as a decimal string, which is why
 * `tsType` for money is `string`.
 */
function mapToDrizzleType(type: string, isArray: boolean): string {
  ...
  const base = typeMap[type.toLowerCase()] ?? 'text';
  return isArray ? `${base}().array` : base;
}
