...
echo "Secrets"
# VITE_* is compiled into the binary. A value that looks like a real
# credential here is one that has already shipped.
if [ -f .env ] && grep -qEi '^VITE_.*(secret|private|password|api_?key)' .env; then
  bad ".env has a VITE_ value that looks like a credential — it ships inside the app"
else
